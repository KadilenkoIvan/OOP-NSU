...
auto operator<<(basic_ostream<Ch, Tr>& os, tuple<Args...> const& t) -> std::basic_ostream<Ch, Tr>& {
    print(os, t);
    return os;
}
...
private:
    ...
    tuple<function<TList(const string&)>...> parser_str = DefaultParsersTuple<TList ... >::get();
...
template <typename ... TList>
const typename CSVReader<TList...>::row_type CSVReader<TList ...>::operator[] (size_t i) const {
    return rows[i];
}
...
int main() {
    ifstream st("D:\\Users\\IvanK\\Desktop\\OOP\\lab4\\train.csv");
    CSVReader<int, int, int, string, string, int, int, int, string, string, string, string> reader(st, 1, ';');
    for (tuple<int, int, int, string, string, int, int, int, string, string, string, string> i : reader) {
        cout << i << endl;
    }
}